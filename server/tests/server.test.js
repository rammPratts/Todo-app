...
describe("DELETE /todos/:id", () => {
  ...
  it("should return 404 if todo not found",(done) => {
    var hexId =  new ObjectID()._id.toHexString();
    request(app)
      .delete(`/todos/${hexId}`)
      .set("x-auth",users[1].tokens[0].token)
      .expect(404)
...
describe("POST /users/signup", () => {
  ...
  var invalidPass = "!pass";
  var usedEmail = users[1].email;
...
describe("POST /users/login", () => {
  ...
  it("should return 400 if login is invalid", (done) => {
    request(app)
    .post("/users/login")
    .send({
      email: users[1].email,
      password: "thispasswordisnotvalid"
    })
    .expect(400)
    .expect((res) => {
      expect(res.headers["x-auth"]).toBeFalsy();
